    ...
    <div class="grid lg:grid-cols-2 gap-8 items-stretch">
      <section class="hidden lg:block relative overflow-hidden rounded-sm border border-sand-200 bg-white shadow-sm">
        ...
        <div class="relative p-8 h-full flex flex-col justify-end">
          ...
          <h1 class="font-serif text-4xl text-white leading-tight">Crie sua conta</h1>
          <p class="mt-3 text-sand-100/90">Cadastre-se e você já entra automaticamente.</p>
        </div>
      </section>

      <section class="bg-white border border-sand-200 rounded-sm shadow-sm p-6 md:p-8">
        <span class="text-wine-700 tracking-widest uppercase text-[10px] md:text-xs font-semibold mb-2 block">Cadastro</span>
        <h2 class="font-serif text-3xl text-stone-900 mb-2">Criar conta</h2>
        <p class="text-sm text-stone-500 mb-6">Informe seus dados para acessar as áreas restritas.</p>

        <c:if test="${not empty error}">
          <div class="mb-6 p-4 rounded-sm bg-red-50 text-red-800 text-sm border border-red-200">${error}</div>
        </c:if>

        <form method="post" action="${pageContext.request.contextPath}/auth/cadastro" class="space-y-4">
          <div>
            <label class="block text-xs font-semibold uppercase tracking-wide text-stone-500 mb-2">Nome</label>
            <input name="nome" required class="w-full border border-sand-300 rounded-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-wine-300 focus:border-wine-500" placeholder="Seu nome" />
          </div>
          <div>
          ...
          <button class="w-full bg-wine-800 hover:bg-wine-900 text-white py-3 rounded-sm font-medium transition-colors">Cadastrar e entrar</button>
        </form>

        <div class="mt-6 text-sm text-stone-600">
          Já tem conta?
          <a class="text-wine-800 underline" href="${pageContext.request.contextPath}/auth/login">Ir para login</a>
        </div>
      </section>
    </div>
  </div>
